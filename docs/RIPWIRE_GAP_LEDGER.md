...
| Receiver-aware resolution | **Now:** RecvKind + conservative resolve + **one-hop type narrowing** + **one-hop field-type narrowing** (Python, TypeScript, Java, Go, **Rust**). Unique `x = Order()` / `x: Order` / `new Foo()` / typed params pin `x.m()` to `{Type}.m`. Unique class/struct field types pin `self.x.m()` / `this.x.m()` / Go `s.x.m()` when the root is the enclosing type. ≥2 types tombstone. Named var still never sprays. Longer field chains unresolved. Import chains (`db.users.Find`) stay unresolved. | `resolve.h` Rule 1 this/self, Rule 2 var type, Rule 2b field type (C++), Rule 3 include-file. | Include-file (3) still deeper for C++ unprefixed fields. | Conservative incomplete edges + provenance; Python/TS/Java/Go/Rust field hop; binds never FILE attrs. | Yes, incrementally | Phase 7 Rule 2 locals/params. Phase 8–11 field-type. Include-file later. | Wrong-but-confident edges. | type-narrow + field-type extract/resolve/index tests. | P1 / Phase 7–11 |
