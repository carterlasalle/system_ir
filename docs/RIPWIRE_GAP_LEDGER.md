...
- Ripwire `fnPtrBindingTarget` is C/C++/ObjC only and fires before Rule 1. SCC analog for Python/TypeScript/Go/Rust: unique extract-time `f = helper` / `const f = helper` / `f := helper` / `let f = helper` pins `RecvKind::None` `f()` to the unique in-repo Function (local, then import, then unique-across-repo) and never falls back to a same-named global. Lambda/arrow/closure is an empty-target bind that blocks the name ladder. A later non-function assignment clobbers only when a fn bind already exists. Type-copy ident RHS is not a fn bind but clobbers a prior one. File-scope empty-scope binds apply when the caller has no local bind; local vs file disagreement refuses. Two same-named Functions across files stay unresolved. Classes are never pins. Binds are extract-time only (never FILE attrs). No C++ extractors. EXTRACTED provenance. Production ranking, MCP tool count, and context levels unchanged.
...
25. Function-alias binds for bare `f()` (Python/TS/Go/Rust analog of Ripwire L3 fn-pointer)
