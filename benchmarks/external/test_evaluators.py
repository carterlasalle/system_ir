...
class TestMatrixWiring(unittest.TestCase):
    ...
    def test_raw_receives_no_artifact(self):
        ...
        self.assertIn("NATIVE_SCC_VARIANTS", src)
        self.assertIn("EXTERNAL_VARIANTS", src)
        self.assertIn("NATIVE_SCC_VARIANTS = ", inspect.getsource(rwm))
...
_RUNPY_ROOT = Path(__file__).resolve().parents[2]
_RUNPY_EVALUATOR = _RUNPY_ROOT / "benchmarks" / "evaluators" / "run.py"
_RUNPY_TASKS_JSON = _RUNPY_ROOT / "benchmarks" / "tasks.json"
SCOPED = [
...
def _runpy_copy_fixture(src: Path) -> Path:
...
def _runpy_run_eval(task_id: str, repo: Path) -> tuple[bool, str]:
    proc = _subprocess.run(
        [sys.executable, str(_RUNPY_EVALUATOR), task_id],
        cwd=repo,
        capture_output=True,
        text=True,
        timeout=45,
...
class RunPyBehavioralEvaluatorsTest(unittest.TestCase):
    """The parallel session's benchmarks/evaluators/run.py suite (21-task
    coverage): identical contract — untouched and comment-only patches
    MUST fail. Merged here so one file enforces both evaluator systems."""
    def test_untouched_fixtures_fail_all_six_scoped(self) -> None:
        for task_id, repo_name, _comment in SCOPED:
            with self.subTest(task=task_id):
                repo = _runpy_copy_fixture(FIXTURES / repo_name)
                try:
                    ok, detail = _runpy_run_eval(task_id, repo)
                    self.assertFalse(
                        ok,
    ...
    def test_comment_only_patches_fail_all_six(self) -> None:
        ...
        for task_id, repo_name, comment in SCOPED:
            repo = _runpy_copy_fixture(FIXTURES / repo_name)
            try:
                _runpy_comment_only(repo, comment)
                ok, detail = _runpy_run_eval(task_id, repo)
                if ok:
    ...
    def test_real_health_implementation_passes(self) -> None:
        repo = _runpy_copy_fixture(FIXTURES / "http-service-python")
        try:
            target = repo / "main.py"
            ...
            ok, detail = _runpy_run_eval("http-service.health-check", repo)
    ...
    def test_every_canonical_task_has_evaluator(self) -> None:
        ...
        src = _RUNPY_EVALUATOR.read_text(encoding="utf-8")
        missing = []
        for t in tasks:
            tid = t["id"]
            if f'"{tid}"' not in src:
