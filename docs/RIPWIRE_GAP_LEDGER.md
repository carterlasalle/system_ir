...
| Unresolved vs external | **Now:** relative import miss → `ImportTarget::Unresolved` (no EXTERNAL_API edge). **Rust `crate::` / `super::` / `self::` / `mod:x` miss from a `.rs` caller is Unresolved** (project-looking). Bare miss → Unknown, not External. Confirmed external import → EXTERNAL_API. `std::` and bare crates stay External. | `externalCalls` is veto, not "name missing". Unresolved often dropped silently. | Operational gauges in headers. | Distinct enum (`ResolutionClass`) + provenance. | Yes | Persist gauges; never label unresolved as external. | Agents treating all miss as stdlib. | quality counts; relative import test; Rust crate:: degrade. | P2 / Phase 1 + 27 |
