...
| Language adapters | Scan classifies py/ts/js/go/rs/java **and C/C++** (SemanticDeep extractors) + config/infra/docs + shell/sql **and** ObjC/C#/Ruby/PHP/Lua/Swift/Kotlin as IndexSearch (no extractor). C/C++ quote-include Step-A; ObjC stays classified-only. | Broad tree-sitter surface including C/C++/ObjC/C#/Ruby/PHP/Lua/Swift, etc. | Breadth + `.scm` extractors; C++ CHA / fn-ptr / compile_commands `-I` / transitive include closure still deeper. | Honest tiers; config/infra extractors Ripwire does not treat as system entities; C/C++ fixtures without claiming ObjC. | Yes (honest matrix now; remaining languages later) | `LANGUAGE_REGISTRY` + `scc languages`. Do not claim support from parse-only. Add languages only with fixtures. | Fake "supported" claims. | Registry uniqueness; scan↔registry; extracted ⇒ tier A. | P10 / Phase 1 SoT, Phase 4 classification, Phase 30 C/C++ |
...
| Receiver-aware resolution | **Now:** RecvKind + conservative resolve + **one-hop type narrowing** (Python, TypeScript, **Go, Rust**, Java locals/params) + **one-hop field-type narrowing** (Python, TypeScript, Java, Go, **Rust**) + **Java unprefixed field-as-receiver** + **Rule 2c class-name receiver** + **Rule 2c CHA/base walk** + **Rule 1 self/this/super base walk** + **Rust `impl Trait for T` CHA** + **function-alias binds for bare `f()`** (Python/TS/Go/Rust) + **Rule 3 import/include-file narrowing**. Unique `x = Order()` / `x: Order` / `new Foo()` / typed params **and Go `x := &Order{}` / Rust `let x = Order {}`** pin `x.m()` to `{Type}.m`. Unique class/struct field types pin `self.x.m()` / `this.x.m()` / Go `s.x.m()` when the root is the enclosing type. Java also pins bare `field.m()` (Ripwire Rule 2b analogue; `.java`-gated). **Go `v.(*Order)` / `Order(v)`, Rust `v as Order`, TS `v as Order` / `<Order>v`, and Java `(Order)v` also pin.** **`Cls.m()` (StaticType) pins the unique in-repo Class/Interface/Type that uniquely defines `m`; a typed local/param named `Cls` wins; an untyped param vetoes; two defining same-named classes stay unresolved. If `Cls` does not define `m`, a BFS of simple-ident bases (cap 16) pins the unique hitting base at the shallowest level (`IERS_B.open()` → `IERS.open`); two hitting bases at one level stay unresolved; two same-named class-likes refuse heritage merge. Heritage is persisted on CLASS entity attributes, never FILE. Modules are not class-like.** **`self.m()` / `this.m()` that is not a sibling of the enclosing class pins via unique class-like plus CHA (`IERS_B.run` `self.open` → `IERS.open`); own-class sibling still wins. `super.m()` and Python `super().m()` never pin the enclosing class (bases only). Two hitting bases at one level stay unresolved (no Ripwire `unionOnMulti` / first-declared MRO). Field chains are not Rule 1. No C++ bare-name Rule 1.** **Rust `impl Open for IERS_B` records Open as a base of IERS_B; trait methods are `Trait.m`; typed `x.open()` pins the unique hitting trait method when T does not define it; inherent impl wins; two traits defining `m` stay unresolved.** **Unique extract-time `f = helper` / `const f = helper` / `f := helper` / `let f = helper` pins bare `f()` to the unique in-repo Function (local, then import, then unique-across-repo) or Const that has a signature (TypeScript function-valued `const helper = () => {}`; `const LIMIT = 10` is not a pin). Do not retag arrows as Function. Never fall back to a same-named global. Lambda/arrow/closure is an empty-target bind that blocks the name ladder. A later non-function assignment clobbers only when a fn bind already exists. Type-copy ident RHS is not a fn bind but clobbers a prior one. File-scope empty-scope binds apply when the caller has no local bind; local vs file disagreement refuses. Two same-named Functions across files stay unresolved unless Rule 3 narrows. Classes are never pins. No C++ extractors.** **Rule 3: path-precise `resolve_import` Internal files only (unresolved/external contribute nothing). Bare `helper()` pins when exactly one imported file defines that callable; same-file defs stay on the local ladder; 0 or ≥2 imported defining files stay unresolved; never basename-guess (`pkg.a` → `pkg/a.py`, not `other/a.py`). Unique imported file still pins when it is the only in-repo def (SCC has no unique-across-repo bare-name ladder). Direct imports only — not Ripwire's transitive C include closure, not `go.mod`, not Java namespaces. **Go/Java Step-A:** `.go` callers unique-or-degrade a package (`{path}.go` xor production files in `{path}/`; multi-file packages expand to one Internal per file so Rule 3/`pkg.m()` see the whole package; `*_test.go` excluded; never steal `.py`/`.ts`; no module-path suffix guessing). `.java` callers unique-or-degrade `com.foo.Bar` to `{path}.java` (plus `src/main/java` / `src/test/java`); star imports contribute nothing; never steal `.py`/`.ts`.** ≥2 types tombstone. **Go `s.owned = &Invoice{}` and Rust `self.owned = Invoice {}` are extract-time tombstone fuel** against a declared Order field. Named var still never sprays. Longer field chains unresolved. Import chains (`db.users.Find`) stay unresolved. | `resolve.h` Rule 1 this/self, Rule 2 var type, Rule 2b field type (C++), Rule 2c class-name, Rule 3 include-file. L3 `fnPtrBindingTarget` is C/C++/ObjC only. | C++ extractors, C++ bare-name Rule 1, C++ fn-pointer address-of/escape, `unionOnMulti` super MRO, and transitive C include closure still deeper. | Conservative incomplete edges + provenance; Python/TS/Java/Go/Rust field hop and local type; Java unprefixed field; Go/Rust assignment tombstone; assertion/cast fuel including Java `(Order)v`; class-name receiver with local-shadow veto and unique-base CHA; Rule 1 self/this/super CHA without spraying multi-base ties; Rust trait-impl CHA; function-alias binds for Py/TS/Go/Rust; Rule 3 path-precise import-file narrow; binds never FILE attrs. | Yes, incrementally | Phase 7–26. C++ extractors later. | Wrong-but-confident edges. | type-narrow + field-type + unprefixed + assignment-tombstone + cast + class-name + CHA + Rule 1 + Rust trait-impl + fn-alias + Rule 3 tests. | P1 / Phase 7–26 |
| Unresolved vs external | **Now:** relative import miss → `ImportTarget::Unresolved` (no EXTERNAL_API edge). **Rust `crate::` / `super::` / `self::` / `mod:x` miss from a `.rs` caller is Unresolved** (project-looking). **Python/TS Step-A unique-or-degrade:** two in-repo hits (`pkg.py`+`pkg/__init__.py`, `x.ts`+`x/index.ts`) are Unresolved, not a guess; Python absolute miss stays External; TS bare (`react`) stays External even if `react.ts` exists. **Go/Java Step-A unique-or-degrade:** `import "fmt"` stays External even if `fmt.py` exists; `pkg.go`+`pkg/` and two package dirs are Unresolved; a unique multi-file Go package expands (does not pick a representative file); Java star `com.foo.*` is Unresolved; two `Bar.java` paths are Unresolved; Java type miss stays External. Bare miss → Unknown, not External. Confirmed external import → EXTERNAL_API. `std::` and bare crates stay External. | `externalCalls` is veto, not "name missing". Unresolved often dropped silently. | Operational gauges in headers. | Distinct enum (`ResolutionClass`) + provenance. | Yes | Persist gauges; never label unresolved as external. | Agents treating all miss as stdlib. | quality counts; relative import test; Rust crate:: degrade; Python pkg.py+init degrade; TS file+index degrade; Go fmt.py steal; Java star/two-file degrade. | P2 / Phase 1 + 27–29 |
...
- ObjC/C#/Ruby/… extractors — classification≠support; C/C++ extractors landed in Phase 30 with fixtures. Do not claim remaining IndexSearch languages. No C++ CHA, fn-pointer address-of/escape, `compile_commands.json` `-I`, or transitive include closure.
...
- C/C++/ObjC/C#/Ruby/PHP/Lua/Swift/Kotlin were **scan-classified** as IndexSearch. Phase 30 promotes **C and C++** to SemanticDeep extractors with quote-include Step-A. ObjC and the rest stay IndexSearch. Classification is not an extractor. No `.scm` rewrite of semantic walkers.
...
- Measured 21-task locator and explore runs with live Ripwire (`benchmarks/results/ripwire-lessons-{locator,explore}-loop.json`): tests_tasks=7; clustered_tests baseline 0.000, SCC **1.000**, Ripwire 0.000 after the ASCII case-fold matcher (7/7 gold-test tasks). File localization unchanged (SCC 1.000, baseline 0.948, Ripwire 0.722).
...
- Remeasured after ASCII case-fold (`API` vs `api`): clustered_tests SCC **1.000** vs baseline/Ripwire 0.000 (7/7 gold-test tasks, including ts-api-web.contract-field and creation-test). Packs already listed `joins user names from the API response`; the matcher was the gap.
...
<!-- trace:exempt reason=document-structure -->
## Phase 25 started (function-alias binds for bare `f()`)

- Ripwire `fnPtrBindingTarget` is C/C++/ObjC only and fires before Rule 1. SCC analog for Python/TypeScript/Go/Rust: unique extract-time `f = helper` / `const f = helper` / `f := helper` / `let f = helper` pins `RecvKind::None` `f()` to the unique in-repo Function (local, then import, then unique-across-repo) or Const that has a signature (TypeScript function-valued `const helper = () => {}`; `const LIMIT = 10` is not a pin). Do not retag arrows as Function. Never fall back to a same-named global. Lambda/arrow/closure is an empty-target bind that blocks the name ladder. A later non-function assignment clobbers only when a fn bind already exists. Type-copy ident RHS is not a fn bind but clobbers a prior one. File-scope empty-scope binds apply when the caller has no local bind; local vs file disagreement refuses. Two same-named Functions across files stay unresolved unless Rule 3 narrows. Classes are never pins. Binds are extract-time only (never FILE attrs). No C++ extractors. EXTRACTED provenance. Production ranking, MCP tool count, and context levels unchanged.

<!-- trace:exempt reason=document-structure -->
## Phase 26 started (Rule 3 import/include-file narrowing)

- Ripwire `rule3IncludeFile` analog on path-precise `resolve_import` Internal files (unresolved/external contribute nothing). Bare `helper()` pins when exactly one imported file defines that callable. Same-file defs stay on the local ladder. 0 or ≥2 imported defining files stay unresolved. Never invent a target. Never basename-guess (`from pkg.a import foo` → `pkg/a.py`, not `other/a.py`). Unique imported file still pins when it is the only in-repo def (SCC has no unique-across-repo bare-name ladder). Function-alias `unique_function_id` uses the same Rule 3 after local/named-import, before unique-across-repo. Direct imports only — not Ripwire's transitive C include closure, not `go.mod` Step-A, not Java namespaces, not C++ extractors. EXTRACTED provenance. Production ranking, MCP tool count, and context levels unchanged.

<!-- trace:exempt reason=document-structure -->
## Phase 27 started (Rust Step-A unique-or-degrade imports)

- Ripwire `resolveRustImport` analog, gated on `.rs` callers so Python/TS/Go do not steal `.rs` files. `use crate::a::b` / `super::` / `self::` and body-less `mod x;` resolve to exactly one `.rs` or `/mod.rs`, or contribute nothing. Both `a.rs` and `a/mod.rs` degrade (never guess). Crate root is the directory of the first `lib.rs`/`main.rs` in sorted file paths; without it, `crate::` is Unresolved. `std::` and bare crates stay External. Brace groups degrade. Body-less `mod geo;` emits `mod:geo`; inline `mod inner { }` does not. No `go.mod`, no C++ extractors, no Java namespaces. EXTRACTED provenance. Production ranking, MCP tool count, and context levels unchanged.

<!-- trace:exempt reason=document-structure -->
## Phase 28 started (Python + TypeScript Step-A unique-or-degrade imports)

- Ripwire `resolvePythonImport` / `resolveTsImport` analog, gated on caller extension. Python probes `mod.py` then `mod/__init__.py` (never `.ts`/`.js`). Relative leading-dot imports (`from .rel`, `from ..models`) probe the includer dir only — `from ..models` pins `pkg/models.py`, not `pkg/sub/models.py`. Absolute imports unique-or-degrade across file-relative, repo-root, and SCC source-root fallbacks: only `src/util.py` still pins; `util.py` and `src/util.py` degrade; `pkg.py` and `pkg/__init__.py` degrade. Zero absolute hits stay External; two-plus in-repo hits stay Unresolved. TypeScript relative `./x` / `../a/b` probe exact, then `.ts`/`.tsx`/`.d.ts`/`.js`/`.jsx`/`.mjs`/`.cjs`, then index files. Bare specifiers (`react`) stay External even if `react.ts` exists. `x.ts` and `x/index.ts` degrade. Python does not steal `web/index.ts`; TS does not steal `util.py`. No tsconfig aliases, no `go.mod`, no C++ extractors, no Java namespaces. EXTRACTED provenance. Production ranking, MCP tool count, and context levels unchanged.

<!-- trace:exempt reason=document-structure -->
## Phase 29 started (Go + Java language-gated Step-A)

- Ripwire `includeLangOf` leaves Go single-root deferred (`kNoFile`) and has no Java file resolver (namespaces ≠ files). SCC analog, gated on caller extension so `.go`/`.java` never steal `.py`/`.ts` via the generic first-match resolver. Go: unique `{path}.go` xor production `.go` files directly in `{path}/`, plus unique SCC source-root fallback; `{path}.go` and `{path}/` together degrade; two package dirs degrade; a unique multi-file package expands to one Internal per production file so Rule 3 and `pkg.m()` unique-or-degrade across the package (last-write must not win); `*_test.go` is not a production package file; relative `./`/`../` only; no `go.mod`, no module-path suffix guessing (`github.com/x/y` ↛ `y.go`). Zero hits stay External. Java: type import `com.foo.Bar` → unique `{dots/to/Type}.java` including `src/main/java` and `src/test/java`; two `Bar.java` paths degrade; star `com.foo.*` contributes nothing; static `com.foo.Bar.BAZ` may pin the parent type file. No Java namespace-as-file. EXTRACTED provenance. Production ranking, MCP tool count, and context levels unchanged.

<!-- trace:exempt reason=document-structure -->
## Phase 30 started (C/C++ extractors + path-precise quote-include Step-A)

- Ripwire CFamily `joinNormalizeLookup(includerDir, target)` analog, gated on C/C++ extensions. Promote C (`.c`/`.h`) and C++ (`.cc`/`.cpp`/`.cxx`/`.hpp`/`.hh`/`.hxx`) from IndexSearch to SemanticDeep with fixture-backed tree-sitter extractors: functions (including prototypes), C++ class methods, calls, `#include`. Quote `#include "foo.h"` is an exact lexical join with the includer directory — hit pins, miss is Unresolved, never basename-guess `other/foo.h`. Angle `#include <stdio.h>` stays External even if a same-named header exists in-repo. Direct includes only — not Ripwire transitive include closure, not `compile_commands.json` `-I`, not C++ CHA, not fn-pointer address-of/escape, not ObjC, not `.scm` rewrite. C/C++ never steal `.py`/`.ts`. Rule 3 pins a unique included defining header. EXTRACTED provenance. Production ranking, MCP tool count, and context levels unchanged.
...
25. Function-alias binds for bare `f()` (Python/TS/Go/Rust analog of Ripwire L3 fn-pointer); TypeScript function-valued consts (`const helper = () => {}`) pin via unique Const+signature, not by retagging arrows as Function
26. Remeasure locator/explore tests_to_run after ASCII case-fold (SCC clustered_tests 1.000)
27. Rule 3 import/include-file narrowing (path-precise Internal imports; unique imported defining file pins; both/neither/same-file controls)
28. Rust Step-A unique-or-degrade import resolution (`crate::` / `super::` / `self::` / `mod x;`; never guess `a.rs` vs `a/mod.rs`)
29. Python + TypeScript Step-A unique-or-degrade import resolution (never guess `pkg.py` vs `pkg/__init__.py` or `x.ts` vs `x/index.ts`; no cross-language steal)
30. Go + Java language-gated Step-A unique-or-degrade import resolution (never steal `.py`/`.ts`; never guess `pkg.go` vs `pkg/`; Java type files unique-or-degrade, star imports contribute nothing; no `go.mod`, no Java namespaces)
31. C/C++ extractors with path-precise quote-include Step-A (SemanticDeep; angle includes External; never basename-guess; ObjC stays IndexSearch; no `compile_commands`/`-I`; no CHA/fn-ptr)
