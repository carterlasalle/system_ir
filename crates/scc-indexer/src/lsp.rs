...
impl LspResolver {
    ...
    fn definition_for(
        ...
        // 1-based). Fallbacks cover servers with off-by-one line handling.
...
mod tests {
    ...
    fn pyright_resolves_reexported_member() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path();
        std::fs::create_dir_all(root.join("lib/helper_pkg")).unwrap();
        std::fs::write(root.join("pyrightconfig.json"), r#"{"extraPaths": ["lib"]}"#).unwrap();
        std::fs::write(
            root.join("lib/helper_pkg/__init__.py"),
            "from .impl import helper\n",
        )
        .unwrap();
        std::fs::write(
            root.join("lib/helper_pkg/impl.py"),
            "def helper():\n    return 1\n",
        )
        .unwrap();
        std::fs::write(
