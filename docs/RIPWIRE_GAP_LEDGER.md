...
| Receiver-aware resolution | **Now:** RecvKind + conservative resolve + **one-hop type narrowing** + **one-hop field-type narrowing** (Python, TypeScript, **Java**). Unique `x = Order()` / `x: Order` / `new Foo()` / typed params pin `x.m()` to `{Type}.m`. Unique class field types pin `self.x.m()` / `this.x.m()` (exactly one hop) the same way, including Java field declarations and `this.x = new Foo()`. ≥2 types tombstone. Named var still never sprays. Longer field chains unresolved. | `resolve.h` Rule 1 this/self, Rule 2 var type, Rule 2b field type (C++), Rule 3 include-file. | Include-file (3) still deeper for C++ unprefixed fields. | Conservative incomplete edges + provenance; Python/TS/Java `self`/`this` field hop; binds never FILE attrs. | Yes, incrementally | Phase 7 Rule 2 locals/params. Phase 8 Rule 2b analogue for self/this. Phase 9 Java. Include-file later. | Wrong-but-confident edges. | type-narrow + field-type extract/resolve/index tests. | P1 / Phase 7–9 |
