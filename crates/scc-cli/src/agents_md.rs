...
/// Replace the managed SCC section in `existing` with `new_section`,
...
pub fn replace_scc_section(existing: &str, new_section: &str) -> String {
    let open_idx = existing.find(SCC_SECTION_OPEN);
    let close_idx = existing.find(SCC_SECTION_CLOSE);
    let (before, after) = match (open_idx, close_idx) {
        (Some(start), Some(end)) if end >= start => {
            let before = existing[..start].trim_end();
            let after = existing[end + SCC_SECTION_CLOSE.len()..].trim_start();
            (before, after)
        }
        (Some(start), None) => {
