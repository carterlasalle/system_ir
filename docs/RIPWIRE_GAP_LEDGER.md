...
| Receiver-aware resolution | **Now:** RecvKind + conservative resolve + **one-hop type narrowing**. Unique `x = Order()` / `x: Order` / `new Foo()` / typed params pin `x.m()` to `{Type}.m` locally or on the imported type. ≥2 types tombstone. Named var still never sprays. Field chains unresolved. | `resolve.h` Rule 1 this/self, Rule 2 var type, Rule 2b field type (C++), Rule 3 include-file. | Field-type (2b) and include-file (3) still deeper for C++. | Conservative incomplete edges + provenance; binds never FILE attrs. | Yes, incrementally | Phase 7: Rule 2 locals/params. Field-type later. | Wrong-but-confident edges. | type-narrow local/tombstone/import/extract tests. | P1 / Phase 7 |
